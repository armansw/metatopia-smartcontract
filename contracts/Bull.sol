...
contract Bull is IGame, ERC721Enumerable, Ownable, Pausable, Whitelisted {
    // mint price
    ...
    constructor(
        ...
        // Runner
        claimAmount[1] = 15 ether;
        // Bull
        claimAmount[2] = 20 ether;
        // Matador
        claimAmount[3] = 0 ether;
        //Alpha
        claimAmount[4] = 35 ether;

        unstakeRiskRate[1] = 1000;// Runner
        unstakeRiskRate[2] = 0; // Bull
        unstakeRiskRate[3] = 0; // Matador
    ...
    function addStakedToken(uint16 _tokenId) external onlyPool override  {
    ...
    function removeStakedToken(uint16 _tokenId) external onlyPool override  {            
    ...
    function distributeHarvestRewards(uint256 _claimedAmountFrom, uint16 _fromTokenId) external override onlyPool {
        require(false, 'No harvest concept at this game.');
    }
    ...
    function getClaimAmountBesideHarvestTax(uint256 _claimAmount, uint16 _tokenId) external view override returns (uint256) {
        require(false, "No harvest concept");
    }
    ...
    function getHarvestRiskDistributeRateForTokenId(uint16 _tokenId) external view returns (uint256) {
        require(false, "No harvest concept");
    }
